...
function install_peass_suite() {
  if [ ! -d "/opt/PEASS-ng" ]; then
    run_with_progress "Cloning PEASS-ng suite" bash -c "git clone https://github.com/carlospolop/PEASS-ng.git /opt/PEASS-ng"
  else
...
PYTEST
  then
    ...
    ensure_venv
    run_with_progress "Installing Impacket in venv" bash -c "source $VENV_DIR/bin/activate && pip install --upgrade impacket"
  fi
}
...
# EXTRA TOOLS
#-----------------------------------------
function install_proxychains() { check_and_install proxychains4 "Proxychains" apt install -y proxychains4; }
function install_filezilla()   { check_and_install filezilla "FileZilla" apt install -y filezilla; }
function install_rlwrap()      { check_and_install rlwrap "rlwrap" apt install -y rlwrap; }
function install_nuclei()      { check_and_install nuclei "Nuclei" bash -c "go install -v github.com/projectdiscovery/nuclei/v2/cmd/nuclei@latest"; }
function install_subfinder()   { check_and_install subfinder "Subfinder" bash -c "go install -v github.com/projectdiscovery/subfinder/v2/cmd/subfinder@latest"; }
function install_feroxbuster() { check_and_install feroxbuster "Feroxbuster" apt install -y feroxbuster; }
function install_ncat()        { check_and_install ncat "Ncat" apt install -y ncat; }
function install_remmina()     { check_and_install remmina "Remmina" apt install -y remmina; }
function install_xfreerdp()    { check_and_install xfreerdp "FreeRDP (xfreerdp)" apt install -y freerdp2-x11; }
...
function install_enum4linux()  { check_and_install enum4linux "Enum4linux" apt install -y enum4linux; }
function install_linpeas() {
  install_peass_suite
  run_with_progress "Linking LinPEAS script" bash -c "ln -sf /opt/PEASS-ng/linpeas/linpeas.sh /usr/local/bin/linpeas"
}
function install_winpeas() {
  install_peass_suite
  run_with_progress "Linking WinPEAS executable" bash -c "ln -sf /opt/PEASS-ng/winPEAS/bin/winPEASexe.exe /usr/local/bin/winpeas"
}
...
function show_menu() {
  dialog --clear --title "Catana Main Menu" \
    --menu "Choose an option:" 26 70 24 \
    1 "Fix Missing Tools" \
    2 "Fix Samba config" \
    3 "Fix Golang env" \
